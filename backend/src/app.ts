...
app.use("*", async (c, next) => {
  ...
  // The script-src hash matches the inline anti-FOUC theme/world script in
  ...
  c.res.headers.set(
    ...
    "default-src 'self'; script-src 'self' 'sha256-rjP2AJO/bKBYoP0JrBCooA+0zLPQ7ogLYjVmw+oKYu0='; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data:; connect-src 'self'; font-src 'self' https://fonts.gstatic.com; object-src 'none'; frame-ancestors 'none'"
  );
});
